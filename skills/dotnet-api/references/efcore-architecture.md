...
> **Quick Ref**: DbContext lifetime is Scoped | Don't wrap with Repository | IDbContextFactory<T> for BackgroundService/Blazor | Migration SQL must be reviewed | Don't create navigation properties across aggregate roots | Don't run migrations at application startup |
...
## Repository Policy: Do Not Wrap EF Core

**Do not create Repository or Unit of Work wrappers around EF Core.** The framework already provides these abstractions:

- `DbSet<T>` **is the repository.** It supports `Add`, `Remove`, `Find`, `Where`, `Include`, `Select`, and all other query patterns.
- `SaveChangesAsync()` **is the Unit of Work.** It batches all tracked changes in a single transaction.
- `DbContext` **is the transaction boundary.** One context per logical unit of work.

Wrapping these in `IRepository<T>` / `IUnitOfWork` interfaces adds zero value and actively harms the codebase:

- Obscures EF Core features (`AsNoTracking`, `AsSplitQuery`, `Include` chains, projections, compiled queries)
- Creates a leaky abstraction that eventually exposes `IQueryable` or requires a new method per query shape
- Adds boilerplate files that must be touched for every new entity
- Makes it harder to reason about database behavior (what is tracked? when does save happen?)

### Direct DbContext Access (The Only Way)
...
If a service grows complex, extract a domain service that takes `DbContext` directly — still no repository interface.

```csharp
// Domain service: handles business logic, injected with DbContext directly
public sealed class OrderService(AppDbContext db)
{
    ...
    public async Task<List<OrderSummary>> GetCustomerOrdersAsync(string customerId, CancellationToken ct)
    {
...
- `IRepository<T>` / `Repository<T>` — generic wrappers
- `IOrderRepository` / `OrderRepository` — per-aggregate wrappers
- `IUnitOfWork` — `SaveChangesAsync()` is UoW
- Extension methods on `DbSet<T>` that re-wrap queries — use the `DbSet` directly
