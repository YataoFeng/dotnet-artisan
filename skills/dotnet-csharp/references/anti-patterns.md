...
> **Quick Ref**: DateTime.Now→TimeProvider | Scoped in Singleton→IServiceScopeFactory | async void→BackgroundService | .Result→await | IRepository→DbContext | string concat→StringBuilder | lock(this)→private object | new HttpClient()→IHttpClientFactory | Tuple→record | 1 impl→no interface
