...
> **Quick Ref**: .NET 10 AddValidation() source-generated | DataAnnotations by default | IValidatableObject for cross-property | Don't use FluentValidation | [GeneratedRegex] prevents ReDoS | Validate file uploads by Magic Bytes, don't trust Content-Type |
...
- **net10.0+ projects: .NET 10 Built-in Validation (`AddValidation`)** — the only option. Source-generator-based, AOT-compatible, auto-discovers types from Minimal API handlers. Use `[ValidatableType]` + `partial class` + Data Annotations attributes.
- **net8.0/net9.0 projects: Data Annotations + endpoint filter** — use `Validator.TryValidateObject()` in a generic endpoint filter. See "Generic Data Annotations Filter" section below.
- **FluentValidation is deprecated for new code.** Do not add `FluentValidation` to new projects. It is only maintained for legacy net8.0 projects with existing validators that cannot be migrated yet.

Do not mix multiple frameworks in the same request DTO.
...
## FluentValidation (Legacy Only)

FluentValidation is deprecated for new .NET 10+ projects. Use `AddValidation()` + DataAnnotations or `[ValidatableType]` source generators instead. See the .NET 10 Built-in Validation section at the top of this file for the preferred approach.

**Only maintained for legacy net8.0 projects** with existing validators that cannot be migrated yet. If you must use it: register with `AddValidatorsFromAssemblyContaining<T>()`, validate manually in handlers (not via deprecated auto-validation pipeline), and return `TypedResults.ValidationProblem(result.ToDictionary())`.
