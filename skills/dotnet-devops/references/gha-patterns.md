...
> **Quick Ref**: Build once, reuse the same Artifact across environments | Add dotnet format --verify-no-changes to CI | Set fail-fast: false for matrix builds | Include runner.os in cache key | Don't deploy different environments with different Artifacts |
