...
AI：没问题，DDD 用在这里的战术模式（聚合、实体、值对象、领域事件），
  不做战略级的限界上下文映射。等团队熟悉了再扩展。
  技术栈方面：.NET 10 + PostgreSQL，需要 CI/CD 和测试吗？

你：要。

行为链：意图检测 → 4 轮提问（领域 → 架构 → 技术 → 质量）→
       捕获领域词汇 → 确定架构（模块化单体 + DDD 战术）→
       拆解领域：[订单、支付、库存] →
...
行为链：意图检测 → 路由到 dotnet-performance-specialist →
...
验证    │ dotnet-testing · dotnet-debugging            →  测试 / WinDbg / 质量清理
