...
flowchart TD
    ...
    F --> G[3. .NET 优先默认]
    G --> H[4. 架构设计<br/>调 architect 代理<br/>选架构 + 文件夹结构 + 构建配置]
    H --> I[5. 分析需求<br/>拆解所需领域]
    I --> I1[后端 API]
    I --> I2[Web 前端]
    I --> I3[PC 桌面端]
    I --> I4[测试]
    ...
    J5 --> K
    J6 --> K
    K --> L[dotnet-workflow<br/>工作流优化 + 学习<br/>记录纠错 · 泛化规则]
```
...
```
你：记住，我们这个项目用 TimeProvider 而不是 DateTime.Now

AI：好的。我捕获到这个规则：在 TimeProvider 构造函数注入优先于 DateTime.Now/UtcNow。
    适用于所有新代码。已记录到 MEMORY.md。

行为链：意图检测 → 路由到 dotnet-workflow →
       检测到纠错 → 泛化为通用规则 → 检查现有记忆 →
       去重后写入 MEMORY.md → 确认已学习
```
...
运维    │ dotnet-devops · dotnet-tooling               →  CI/CD / 版本迁移 / Git 工作流 / 解决方案结构 + 脚手架 / 代码质量
...
| 你说 | 代理 | 专长 |
|------|------|------|
| "这个项目怎么架构？" | architect | 架构选型、解决方案结构、配置体系 |
...
| "记住这个" / "学这个模式" | workflow（技能） | 纠错捕获、模式泛化、记忆 |
