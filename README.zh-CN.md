...
安装插件即自动启用。打开 .NET 项目 → harness 检测到 → 技能加载 → 决策者激活。零配置，无需复制。

| 时机 | 行为 |
|------|--------------|
| 打开 .NET 项目 | 检测 `.cs`/`.csproj`/`.sln` → 自动加载技能 → 显示 .NET 版本 |
| 输入 .NET 提示 | 检测 C#/ASP.NET/Blazor 关键词 → 确认路由 |
| 创建/编辑 `.cs` 文件 | 检查一行用途注释 → 提醒你（30 秒法则） |

**[→ 工作原理](harness/README.md)**
